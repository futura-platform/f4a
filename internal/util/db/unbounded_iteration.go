...
func UnboundedIterate[T fdb.ReadTransactor](
	...
	batchSize int,
) iter.Seq[mo.Either[error, fdb.KeyValue]] {
	return func(yield func(mo.Either[error, fdb.KeyValue]) bool) {
		exhausted := false
		for !exhausted {
			...
			if err != nil {
				yield(mo.Left[error, fdb.KeyValue](err))
				return
			}
			for _, kv := range batch {
				if !yield(mo.Right[error](kv)) {
					return
				}
