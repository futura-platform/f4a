...
func TestWorkLoop(t *testing.T) {
	...
	t.Run("a run whose task is deleted under it is not a run failure", func(t *testing.T) {
		testutil.WithEphemeralDBRoot(t, func(db dbutil.DbRoot) {
			...
			// A delete clears the task's directory (input included) in the same
			// transaction that removes it from the runner's set: the input watch
			// fires before the removal event reaches the work loop.
			tasksDirectory, err := task.CreateOrOpenTasksDirectory(db)
			require.NoError(t, err)
			_, err = db.Transact(func(tx fdb.Transaction) (any, error) {
				taskKey, err := tasksDirectory.Open(tx, id)
